...
kimi-code 将模型适配集中在 `packages/kosong`：`ChatProvider` 规定统一生成与配置接口，provider adapter 分别实现 Kimi、OpenAI Chat、OpenAI Responses、Anthropic、Google GenAI/Vertex，Agent Core 只消费规范化的流式消息部件。研究版本：`bf8e967d5`。
